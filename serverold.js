...
//homepage route
app.get('/', (req, res) => {
    // get name from queery parameters (default to 'World' if no name query param exists)
...
app.get('/friends', (req,res)=>{
    ...
    for (let index = 0; index < data.length; index++) {
        ...
        friends += `<li><a href ="/friends/${friend.handle}">${friend.name}</li>`
    }
...
// friend detail page (uses route parameters indicated by :handle (handle in the data.js))
app.get('/friends/:handle', (req,res)=>{
    ...
    // if we did find a friend 
        ...
        // use the details to send back a page
        res.send(`
        <h1>${friend.name}</h1>
        <h3>${friend.handle}</h3>
        <p>${friend.skill}</p>
